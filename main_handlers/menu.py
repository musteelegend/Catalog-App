...
from login_handlers.oauth_login import getUserInfo
...
def showMenu(catalog_id):
    ...
    if ('username' not in login_session or creator.id !=
        ...
        return render_template('publicmenu.html', count=count, items=items,
                               catalog=catalog, catalogs=catalogs,
                               creator=creator)
    else:
        return render_template('menu.html', count=count, items=items,
                               catalog=catalog, catalogs=catalogs,
                               creator=creator)
...
def showMenuItem(catalog_id, item_title):
    ...
    if ('username' not in login_session or creator.id !=
        login_session['user_id']):
        return render_template('publicmenuitem.html', item=item,
                               catalog=catalog, creator=creator)
    else:
        return render_template('menuitem.html', item=item, catalog=catalog,
                               creator=creator)
...
def newMenuItem():
    ...
    if request.method == 'POST':
        ...
        newItem = MenuItem(title=request.form['title'],
...
# Edit a menu item
@menu_page.route('/catalog/<int:catalog_id>/menu/<string:item_title>/edit',
                 methods=['GET', 'POST'])
def editMenuItem(catalog_id, item_title):
    ...
    if login_session['user_id'] != catalog.user_id:
        return "<script>function myFunction() {alert('You are not authorized to edit menu items to this catalog. Please create your own catalog in order to edit items.');}</script><body onload='myFunction()''>"  # noqa
    if request.method == 'POST':
        ...
        return redirect(url_for('menu_page.showMenuItem',
                                item_title=item_title, catalog_id=catalog_id))
    else:
        return render_template('editmenuitem.html', catalog=catalog,
                               item=editedItem, catalogs=catalogs)


# Delete a menu item
@menu_page.route('/catalog/<int:catalog_id>/menu/<string:item_title>/delete',
                 methods=['GET', 'POST'])
def deleteMenuItem(catalog_id, item_title):
    ...
    if login_session['user_id'] != catalog.user_id:
        return "<script>function myFunction() {alert('You are not authorized to delete menu items to this catalog. Please create your own catalog in order to delete items.');}</script><body onload='myFunction()''>"  # noqa
    if request.method == 'POST':
